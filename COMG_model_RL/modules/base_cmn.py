...
class BaseCMN(AttModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        text_embeddings, out = self.model(att_feats, seq, att_masks, seq_mask, memory_matrix=self.memory_matrix)
        outputs = F.log_softmax(self.logit(out), dim=-1)

        return text_embeddings, outputs
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        if len(state) == 0:
            ...
            past = state[1:]
        _, (out, past) = self.model.decode(memory, mask, ys, subsequent_mask(ys.size(1)).to(memory.device), past=past,

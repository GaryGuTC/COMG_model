...
total_disease_list = ["fracture",
                "pneumonia", "nodule", "opacity" ,"consolidation", "edema", "atelectasis", "lesion", "infiltrate", "mass", "emphysema", "fibrosis", # lung
...
def generate_disease_prompt(embeding_model, tokenizer, batch_size, device):
    ...
    for each_caption in disease_caption:
        for each in each_caption.split(" "):
            ...
            four_caption.append(embeding_model(prompt)) # 4,bs,x,512
    return four_caption
...
class BaseCMNModel(nn.Module):
    def __init__(self, args, tokenizer):
        ...
        # self.disease_token_embedding = cp(self.encoder_decoder.model.tgt_embed)
    ...
    def forward_iu_xray(self, 
        ...
        disease_prompts = generate_disease_prompt(self.encoder_decoder.model.tgt_embed, self.tokenizer, images.shape[0], images.device) #[bs,37,512]
        disease_token_target = self.encoder_decoder.model.tgt_embed(disease_detected) if disease_detected != None else None # bs, x, 512
        ########## generate disease token ##########
    ...
    def forward_mimic_cxr(self, 
        ...
        disease_token_target_mediastinum = self.encoder_decoder.model.tgt_embed(disease_detected_mediastinum) if disease_detected_mediastinum != None else None # bs, x, 512
